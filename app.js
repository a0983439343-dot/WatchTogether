(() => {
  ...
  const PLATFORMS = {
    youtube: { name: "YouTube", icon: "▶", searchable: true, player: "youtube" },
    vimeo: { name: "Vimeo", icon: "▶", searchable: false, player: "vimeo" },
    dailymotion: { name: "Dailymotion", icon: "▶", searchable: false, player: "dailymotion" },
    bilibili: { name: "Bilibili", icon: "📺", searchable: false, player: "bilibili" },
    twitch: { name: "Twitch", icon: "🎮", searchable: false, player: "twitch" },
    netflix: { name: "Netflix", icon: "🔴", searchable: false, player: "external" },
    crunchyroll: { name: "Crunchyroll", icon: "🟠", searchable: false, player: "external" },
    disneyplus: { name: "Disney+", icon: "🔵", searchable: false, player: "external" },
    primevideo: { name: "Prime Video", icon: "🟦", searchable: false, player: "external" },
    appletv: { name: "Apple TV+", icon: "🍎", searchable: false, player: "external" },
    max: { name: "Max", icon: "🟣", searchable: false, player: "external" },
    hulu: { name: "Hulu", icon: "🟢", searchable: false, player: "external" },
    paramount: { name: "Paramount+", icon: "🟦", searchable: false, player: "external" },
    peacock: { name: "Peacock", icon: "🦚", searchable: false, player: "external" }
  };
  ...
  function getVimeoId(value) {
    ...
    return text.match(/vimeo\.com\/(\d+)/)?.[1] || null;
  }
  ...
  function getDailymotionId(value) {
    ...
    try {
      ...
      if (url.hostname.includes("dai.ly")) {
        return (
          url.pathname.replace(/^\/+/, "").split("/")[0] || null
        );
      }
    } catch (_) {}
  ...
  function getBilibiliId(value) {
    ...
    try {
      ...
      const bv = url.pathname.match(/(BV[a-zA-Z0-9]+)/);
  ...
  async function searchYoutube(query) {
    ...
    if (
      ...
      throw new Error("尚未設定 YouTube Data API Key");
    }
  ...
  function renderSearchResults(results) {
    ...
    container.querySelectorAll(".video-result-card").forEach((card) => {
      card.addEventListener("click", () => {
        const id = String(card.dataset.videoId || "");

        const video = state.searchResults.find(
        ...
        state.modalSelectedVideoId = id;

        container.dataset.selectedVideoId = id;

        container.querySelectorAll(".video-result-card").forEach((item) => {
          item.classList.toggle(
            "selected",
            String(item.dataset.videoId || "") === id
          );
        });

        if ($("saveSourceBtn")) {
          $("saveSourceBtn").disabled = false;
          $("saveSourceBtn").textContent = "使用這部影片";
        }
  ...
  function forceYoutubeVisible() {
    ...
    if (!state.youtubeRequestedId && !state.currentVideoId) {
      return;
    }
  ...
  async function destroyCurrentPlayer(options = {}) {
    const keepYoutube =
    ...
    try {
      if (oldType === "youtube") {
        old.destroy?.();
      } else if (oldType === "vimeo") {
        await old.destroy?.();
      } else if (oldType === "dailymotion") {
        old.destroy?.();
      }
    } catch (_) {}
  }
  ...
  async function asyncCurrentPosition() {
    ...
    try {
      ...
      if (type === "vimeo") {
        return Number(await player.getCurrentTime()) || 0;
      }
  ...
  async function asyncDuration() {
    ...
    try {
      ...
      if (type === "vimeo") {
        return Number(await player.getDuration()) || 0;
      }
  ...
  async function writePlayback(action, playing) {
    if (
      !state.stateRef ||
      !state.uid ||
      !state.playerReady ||
  ...
  async function syncLatestPlayback() {
    ...
    try {
      const snapshot =
        await state.stateRef.once("value");
      ...
      await applyRemotePlayback(data, true);
    } catch (error) {
      console.error(
        "讀取播放狀態失敗:",
        error
  ...
  async function applyRemotePlayback(data, force = false) {
    if (!data) {
      return;
    }
  ...
  async function applyPlayerPosition(seconds, force = false) {
    const player = state.player;
    const type = state.playerType;
    ...
    try {
      const local = await asyncCurrentPosition();

      const difference = Math.abs(local - seconds);

      if (!force && difference <= 0.8) {
        return;
      }
  ...
  async function playPlayer() {
    ...
    try {
      ...
      } else if (state.playerType === "vimeo") {
        await state.player.play();
      } else if (state.playerType === "dailymotion") {
        await state.player.play();
      } else if (state.playerType === "twitch") {
        state.player.play();
      }
    } catch (_) {}
  }
  ...
  function createVideoObject(platform, rawValue, title = "") {
    if (!rawValue) {
      return null;
    }

    if (platform === "youtube") {
      const id = getYoutubeId(rawValue);
    ...
    if (platform === "dailymotion") {
      const id = getDailymotionId(rawValue);
    ...
    if (platform === "bilibili") {
      const id = getBilibiliId(rawValue);
    ...
    if (platform === "twitch") {
      const value = getTwitchValue(rawValue);
      ...
      if (existing) {
        ...
        setTimeout(() => {
          ...
          if (window.Vimeo?.Player) {
            ...
            reject(new Error("Vimeo SDK 載入失敗"));
          }
        }, 10000);
  ...
  async function loadDailymotionSdk() {
    ...
    await new Promise((resolve, reject) => {
      ...
      if (existing) {
        ...
        setTimeout(() => {
          ...
          if (window.dailymotion) {
            ...
            reject(
              new Error("Dailymotion SDK 載入失敗")
            );
          }
        }, 10000);
      ...
      script.async = true;
      script.dataset.wtDailymotionSdk = "1";
      ...
      if (existing) {
        ...
        setTimeout(() => {
          ...
          if (window.Twitch?.Player) {
            ...
            reject(new Error("Twitch SDK 載入失敗"));
          }
        }, 10000);
  ...
  async function buildYoutubePlayer(videoId) {
    ...
    const token = ++state.youtubeBuildToken;
          ...
          "youtubePlayer",
          {
            ...
            events: {
              onReady: async () => {
                ...
                setTimeout(forceYoutubeVisible, 100);
                setTimeout(forceYoutubeVisible, 500);
                setTimeout(forceYoutubeVisible, 1500);
              },
  ...
  async function buildVimeoPlayer(video) {
    ...
    player.on("play", async () => {
      if (!state.applyingRemote) {
        await writePlayback("play", true);
      }
    });

    player.on("pause", async () => {
      if (!state.applyingRemote) {
        await writePlayback("pause", false);
      }
    });
  ...
  async function buildBilibiliPlayer(video) {
    await destroyCurrentPlayer();

    hidePlayers();

    const iframe = $("bilibiliPlayer");
    ...
    if (/^BV/.test(video.id)) {
      src =
        ...
        "&page=1&autoplay=0";
    } else if (/^av/i.test(video.id)) {
      src =
        "https://player.bilibili.com/player.html?aid=" +
        encodeURIComponent(
          video.id.replace(/^av/i, "")
        ) +
        "&page=1&autoplay=0";
    } else {
      src =
  ...
  async function buildExternalPlayer(video) {
    await destroyCurrentPlayer();

    hidePlayers();

    const notice = $("platformPlayerNotice");
    ...
    if ($("roomPlatformIcon")) {
      $("roomPlatformIcon").textContent =
        PLATFORMS[video.platform]?.icon ||
        "🌐";
    }

    if ($("roomPlatformName")) {
      $("roomPlatformName").textContent =
        name;
    }

    if ($("roomPlatformDescription")) {
      $("roomPlatformDescription").textContent =
        "此平台請使用官方網站播放。";
    }

    const button = $("openPlatformBtn");
  ...
  async function buildPlatformPlayer(video) {
    if (!video) {
      hidePlayers();

      showPlayerElement("emptyPlayer");

      return;
    }

    if (video.platform === "youtube") {
      await buildYoutubePlayer(video.id);
      return;
    }

    if (video.platform === "vimeo") {
      await buildVimeoPlayer(video);
      return;
    }

    if (video.platform === "dailymotion") {
      await buildDailymotionPlayer(video);
      return;
    }

    if (video.platform === "bilibili") {
      await buildBilibiliPlayer(video);
      return;
    }

    if (video.platform === "twitch") {
      await buildTwitchPlayer(video);
      return;
    }

    await buildExternalPlayer(video);
  }

  async function handleRoomVideo(video) {
    if (!video) {
    ...
    state.room.video = normalized;
    state.room.sourceType = normalized.platform;

    if (normalized.platform === "youtube") {
      const videoId = getYoutubeId(normalized.id);
      ...
      await buildYoutubePlayer(videoId);

      return;
    }

    await buildPlatformPlayer(normalized);
  }
  ...
  async function enterRoom() {
    ...
    state.stateRef =
      db.ref(`rooms/${state.roomId}/state`);

    state.membersRef =
      db.ref(`members/${state.roomId}`);

    state.chatRef =
      db.ref(`rooms/${state.roomId}/chat`);
    ...
    if (!state.stateListenerAttached) {
    ...
    if (!state.membersListenerAttached) {
      state.membersRef.on("value", (snapshot) => {
        renderMembers(
          snapshot.val() || {}
        );
      });

      state.membersListenerAttached = true;
    }

    if (!state.chatListenerAttached) {
      state.chatRef
        .limitToLast(100)
        .on("value", (snapshot) => {
          renderChat(
  ...
  async function changeVideo(video) {
    ...
    state.room.sourceType = platform;
    state.room.video = roomVideo;
  ...
  function openSourceModal() {
    ...
    if ($("modalVideoSearchResults")) {
      $("modalVideoSearchResults").innerHTML =
        "";
    ...
    if ($("saveSourceBtn")) {
      $("saveSourceBtn").disabled =
        false;

      $("saveSourceBtn").textContent =
        "選擇影片";
    }
  ...
  function detachRoomListeners() {
    ...
    state.stateListenerAttached = false;
    state.videoListenerAttached = false;
    state.membersListenerAttached = false;
    state.chatListenerAttached = false;
  }
